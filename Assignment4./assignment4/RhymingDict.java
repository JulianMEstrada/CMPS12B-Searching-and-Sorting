...
// TO DO 2: Store a rhymeGroup (key) and word (value) in the Dictionary (hashtable)
	public static void storeRhyme(DictionaryInterface rhymingDict, String line) {
		// Look up rhyme group in the dictonary, if it is null then the rhyme has not been added before. 
		// Do the following 
		String word = getWord(line);
		String rhymeGroup = getRhymeGroup(line);
		if(rhymingDict.get(rhymeGroup)==null){ // if null create a new list and add the word to the newList, also put in dict
	...
	// removeUnrhymable words
		String [] arr = rhymingDict.getKeys(); 
		// get the list of the keys (rhymingDict.get(arr[i]))
		for(int i=0; i<arr.length;i++){
//System.out.println((rhymingDict.get(arr[i])));
			if(((MyLinkedList)rhymingDict.get(arr[i])).size()<= 1){ // if the size less then one we know a rhyme is not included
				rhymingDict.remove(arr[i]);
				
			} 
		}
	} // end removeUnrhymable words
